...
**Current baselines:**
- Project Brief — **v0.23** (26 Jun 2026)
- Legal Brief — **v0.10** (23 Jun 2026)
- Engineers Brief — **v0.20** (26 Jun 2026)
...
**Targets:** Project Brief v0.24 (§3.69 low-certainty text-to-video evidence card note when implemented). Engineers Brief v0.21 (§3.21/§3.27 subject identity section; §3.69 evidence card note; §3.73 summary copy; §3.74 score mismatch). Trademark Clearance Brief v0.4 (US/EU/UK filing outcomes; entity assignment once incorporated; Class 41 skip confirmed; any phrase mark filing decision).
...
### 🔴 §3.21 + §3.27 Subject identity — EB spec section missing (12 Jun / 17 Jun 2026)

**Status:** 🔴 Open — feature is live in prototype (Phase 1 NLP confirmed working, §3.31 closed); Phase 2 attorney Qs filed in LB v0.9; but Engineers Brief has no §3.x subject identity section

**What's live:** spaCy NER + Wikidata list + wordninja hashtag pre-processing all confirmed working in prototype (§3.31 closed 27 Jun). Phase 2 attorney questions (face recognition, biometric processing basis, EU AI Act) filed in Legal Brief v0.9.

**What's missing from EB:**
No Engineers Brief section documents the subject identity feature — not in §3, §4.4, or §5. The Phase 1 backlog in §13 references "Subject identity Phase 1 (spaCy NER + Wikidata list) — implementation status to be confirmed" but there is no spec section. Needed:
- New EB §3.x — Phase 1 NLP spec: spaCy NER, curated Wikidata list, wordninja hashtag pre-processing (extract `#hashtag` tokens, strip `#`, `wordninja.segment()`, inject into NER input), score as modifier on source reputation not standalone, evidence card copy
- EB §4.4 — subject identity noted as source rep modifier (not fusion pillar)
- EB §13 — Phase 1 backlog item updated to "confirmed live"

**Documents affected:** Engineers Brief v0.21 (new §3.x; §4.4 note; §13 status update).

---

### 🟡 §3.50 Cloudflare production stack — steps 6–7 pending (23 Jun 2026)

**Status:** 🟡 In progress — steps 1–5 complete; steps 6–7 pending

**Decisions locked:**
- Primary domain: **skept.co** (skept.app unavailable)
- Production stack: Cloudflare-native (Pages, Workers, D1, R2, KV). Railway = prototype/dev only.
- Build-product-first / landing-page-last.
- Auth required from day one — no anon analysis path on Cloudflare stack.
- URL ingestion: Railway stays alive as ingestion service (yt-dlp → R2 upload) until replaced.
- Fusion extracted as shared module (`cloudflare/fusion.js`).

**Pricing:** always read `skept-pricing-summary-v1.md` — never recall from memory.

**Steps complete (1–5):**
1. ✅ Auth Worker — `auth-worker.js`, `wrangler-auth.toml`, `skept_d1_schema_auth.sql`. D1 `skept-auth` (ID: b81343be-4703-4d26-959e-46781843d563). KV `AUTH_SESSIONS` (ID: acb86bfb713e453ab616053bdbc785f8). Worker live at skept-auth.c-doust85.workers.dev. Secrets: RESEND_API_KEY, ENCRYPTION_KEY, IP_SALT. Resend verified skept.co (DKIM + SPF, Tokyo).
2. ✅ Tier config — `cloudflare/tier-config.js`; verify Worker refactored.
3. ✅ Verify flow — `ingestion-worker.js`, `cloudflare/fusion.js`, `cloudflare/verify-worker.js`, `cloudflare/wrangler-verify.toml`. D1 `skept-analysis` (ID: f1c17fe3-fd52-48cb-a22c-f4658918e301). Schema confirmed.
4. ✅ History Worker — `cloudflare/history-worker.js`, `cloudflare/wrangler-history.toml`.
5. ✅ Settings Worker — `cloudflare/settings-worker.js`, `cloudflare/wrangler-settings.toml`. Worker live at skept-settings.c-doust85.workers.dev.

**Remaining before verify Worker deploy:**
- R2 bucket SKEPT_CLIPS_BUCKET created in Cloudflare dashboard
- Worker secrets: RESEMBLE_API_KEY, INGEST_SECRET, R2_BUCKET_URL
- Railway env vars: INGEST_SECRET, R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME

**Steps pending:**
6. Billing — Stripe checkout + webhook Workers (web); RevenueCat webhook Worker (mobile). Stripe account live; MCP plugin active in Claude Code. Schema hooks already in place (`subscription_source`, `subscription_ref`, `tier`, `tier_expires_at` in `users` table). Pre-deploy actions: create products/prices in Stripe and RevenueCat dashboards, run D1 migration, deploy Workers, provision secrets via `wrangler secret put`.
7. Landing page swap — `skept-signin-flow.html` replaces `skept-landing.html` at launch.

---

### 🔴 §3.69 Deepfake — certainty scalar suppression on text-to-video content (Jun 2026)

**Status:** 🔴 Open — code fix and EB spec both outstanding

**Context:** On fully synthetic text-to-video content (Biden/alien Reel), Resemble returns `video_metrics.score` ~60% (label: Likely Fake) but `certainty=0.2012`. Certainty-weighted scalar suppresses `final_score` to ~4%, inverting the verdict to Likely Authentic. Scalar is architecturally correct — text-to-video has no real faces so low certainty is accurate. Problem: suppressed score and raw Resemble signal diverge significantly with no explanation surfaced to the user.

**Fix (bundles with §3.73):**
- When `certainty < 0.25` AND `resemble_video_score > 0.50`: add evidence card note — "Low frame-detection confidence — this content may be AI-generated. Visual analysis score has been adjusted accordingly."
- `resemble_video_score` must be present in the `non_human` return path in `deepfake.py` (confirmed missing — was the root cause of the 27 Jun deployment not rendering the note).

**Architectural fix:** §3.20 Sightengine `synthetic_generation` pillar — designed to catch this content class independently of the certainty scalar. Defer until after Resemble pillars are calibrated.

**Documents affected:** Engineers Brief v0.21 §4.6 (verdict display spec — evidence card note for low-certainty text-to-video); Project Brief v0.24 (§4 note).

---

### 🔴 §3.73 UI — deepfake evidence card summary false negative on certainty-suppressed scores (27 Jun 2026)

**Status:** 🔴 Open — bundle with §3.69 fix; confirm deployment ID before sending prompt

**Context:** Evidence card summary reads "Video analysis found no deepfake indicators (7% suspicion score)" when `final_score=0.044` post-certainty-scalar, but Resemble returned `video_metrics.label=Likely fake` at 60.06%. Summary is derived from `final_score` alone — on text-to-video content with low certainty scalars, this diverges sharply from the raw Resemble verdict and produces a false-negative statement visible to users.

**Fix:** Gate summary copy on `video_metrics.label`. When `resemble_video_score > 0.50` AND `final_score < 0.15`, do not assert "no deepfake indicators" — use neutral framing: e.g. "Visual analysis score adjusted for low face-detection coverage."

**Action:** Confirm new deployment ID at next session start (§3.69 fix may not have redeployed — same deployment ID `ba214653` was returned post-commit). Bundle §3.69 + §3.73 into a single Claude Code prompt once deployment is confirmed.

**Documents affected:** Engineers Brief v0.21 §4.6 (evidence card summary copy spec).

---

### 🔴 §3.74 UI — fusion score display mismatch: `final_score=0.0070` renders as 3% (27 Jun 2026)

**Status:** 🔴 Open — audit required before fix

**Context:** @adsleben TikTok AR meme (deployment 7ea434db). Log: `[fusion] score=0.0070 denom=0.6000` → 0.70%. UI verdict banner and deepfake evidence card both display 3%. Arithmetic confirmed correct in fusion layer — mismatch is UI rendering only.

**Possible causes:**
- Evidence card reading a different field than `final_score` (e.g. `video_metrics.score`)
- Percentage display applying a rounding/multiplier step that amplifies at very low values
- Stale `(raw+1)/2` path surviving in the template for the deepfake card specifically

**Action:** Claude Code audit of `main.py` — identify the field read for the deepfake evidence card percentage and the verdict banner percentage. Confirm both match `final_score` from the pillar dict. Do not write a fix until the source field is confirmed.

**Documents affected:** None — UI only.
...
| --- | --- | --- | --- |
| project_brief_v0_23.docx | v0.23 | 26 Jun 2026 | v0.24 — §3.69 low-certainty text-to-video note when implemented |
| legal-brief-v0_10.docx | v0.10 | 23 Jun 2026 | Next attorney engagement outcomes or new legal questions |
| engineers_brief_v0_20.docx | v0.20 | 26 Jun 2026 | v0.21 — §3.21/§3.27 subject identity section; §3.69 evidence card note; §3.73 summary copy spec; §3.74 score mismatch (once audited); §3.70 calibration note in §3.3 |
...
| skept-pricing-summary-v1.md | v1 | Jun 2026 | Update on any pricing change — authoritative source, always read before citing numbers |
