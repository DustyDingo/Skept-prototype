...
def _sample_video(video_path: str, duration: float) -> tuple[str, dict]:
    """Extract 5s from start and 5s centred on midpoint, concat into a single file.
    ...
    b_start = duration / 2.0 - 2.5
    b_end   = b_start + 5.0

    subprocess.run(
        ["ffmpeg", "-ss", "0", "-i", video_path, "-t", "5", "-c", "copy", str(seg_a), "-y", "-loglevel", "error"],
        capture_output=True, timeout=60,
    )
    subprocess.run(
        ["ffmpeg", "-ss", str(b_start), "-i", video_path, "-t", "5", "-c", "copy", str(seg_b), "-y", "-loglevel", "error"],
        capture_output=True, timeout=60,
    )
    ...
    print(
        f"[deepfake] sampled=True strategy=start_mid_5s "
        f"original_duration={duration:.1f} "
        f"seg_a=0-5 seg_b={b_start:.1f}-{b_end:.1f}",
        flush=True,
    )
...
async def run_deepfake(video_path: str) -> dict:
    ...
    try:
        # §3.44/§3.77 — 5s start + 5s mid segment strategy
